...
export default async function BuilderBannerAdsPage() {
  ...
  return (
    <DashboardShellV2
      ...
      mobileDrawerNav={getBuilderNav("/dashboard/builder/ads")}
    ...
    >
      ...
      {!bannerFeatures.paymentRequired && (
        <div className="mb-4 rounded-xl border border-amber-500/20 bg-amber-500/10 px-3.5 py-2.5 text-xs text-amber-700 dark:text-amber-300">
          Paid banner packages are{" "}
          <span className="font-semibold">Setup Required</span> — the Razorpay
          ads flow isn&apos;t enabled yet. Submitted ads go to admin review and
          run free once approved; no payment is taken or faked.
        </div>
      )}
      ...
      {ads.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-surface px-6 py-14 text-center">
          <Megaphone
            className="mb-3 h-9 w-9 text-ink-muted"
            aria-hidden="true"
          />
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex min-w-0 gap-3">
                  {ad.desktop_image_url || ad.mobile_image_url ? (
                    ...
                    <div className="flex h-12 w-20 shrink-0 items-center justify-center rounded bg-surface-muted text-[10px] text-ink-muted">
                      No image
                    </div>
                  )}
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-xs text-ink-muted">
                      {ad.gujarat_wide
                        ? "Gujarat-wide"
                        : ad.targets?.map((t) => t.city_name).join(", ") ||
                          "No city"}
      ...
      {!bannerFeatures.analytics && (
        <p className="mt-5 text-xs text-ink-muted">
          Impression &amp; click analytics will appear here once tracking is
