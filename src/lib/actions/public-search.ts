...
export async function getPublicPropertyBySlug(slug: string) {
  ...
  const { data } = await supabase
    .from("public_properties_view")
    .select(
      "id, title, slug, description, purpose, category, property_type, price, rent_amount, deposit_amount, price_negotiable, area_value, area_unit, built_up_area, carpet_area, plot_area, bedrooms, bathrooms, balconies, floor_number, total_floors, furnishing_status, property_age, possession_status, available_from, facing, parking, amenities, city_text, locality_text, building_name, landmark, pin_code, approx_latitude, approx_longitude, cover_media_id, media_count, status, published_at, poster_role, owner_profile_id, contact_visibility, poster_display_name, poster_member_since, poster_verified"
    )
    .eq("slug", slug)
    .maybeSingle();
  return data;
...
// NOTE: the old getPublicListingDirectPhone (full number resolved into the
// initial render when contact_visibility allowed) was removed in Batch 4 —
// it violated the masked-until-reveal design (spec §7-8). Detail pages now
// use getListingContactState + revealListingContact in
// src/lib/actions/contact.ts instead.
