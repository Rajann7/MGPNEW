...
## 2026-07-10 — Wireframe deep-check (Home / Search / Login)

1. **Search result card showed raw enum text** — chips rendered `1450sq_ft` / `Flat Apartment` (raw `area_unit` + `property_type` values). Fix: `PropertyResultCard.tsx` now uses `formatArea()` + `labelize()`. Status: FIXED, retested live on /search.
2. **Home role-CTA band / trust section / how-it-works / featured-project cards did not match Batch 3** (off-design interim visuals: dark-green split panel, dark project cards, numbered step cards). Fix: all four components rebuilt 1:1 from Batch 3 Screen 1 markup, backend wiring kept. Status: FIXED, verified at 390/1366px.
3. **Noted, not a defect:** desktop header primary-nav/mega-menu row absent — deliberate removal in commit 6b2d59a (2026-07-09) per user simplification; CLAUDE.md §40 nav-row requirement stands superseded for now.
4. **Pending (SETUP_REQUIRED):** search "Map view" toggle from Batch 3 Screen 5 not built (Maps provider not configured; design mandates honest fallback card when it ships).
