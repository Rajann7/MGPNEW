...
## BUG-2026-07-14-01 — Project wizard could not advance past Step 1 (CRITICAL, FIXED, live-retested)
- **Found:** Phase 2 manual verification, driving the Project posting wizard live as testbuilder. Clicking **Continue** on Step 1 (Basics) showed "Something went wrong. Please try again." and never advanced — the project wizard was **completely unusable** (no project could be posted through the UI).
- **Root cause:** The Batch 5 draft migration (`20260710120000`) relaxed `properties.purpose/category/property_type` to allow partial drafts (NOT NULL dropped + a `status='draft'`-conditional CHECK), but the **same relaxation was never applied to `projects`**. `projects.project_type/category/purpose` stayed `NOT NULL`, and `ProjectDraftSchema` required all three as non-optional enums — yet the wizard collects `project_type` only on **Step 2**. So `createProjectDraft` (called by `handleNext` on Step 1) always failed: Zod rejected the empty `project_type`, and even past Zod the DB insert would violate NOT NULL. The property wizard avoided this because `PropertyDraftSchema` makes these `.nullable().optional()` and the properties table was relaxed.
- **Severity:** Critical (a whole role's primary create-flow was dead — Builders could not post projects; CLAUDE.md §NO DEAD UI / "incomplete feature shown as working").
- **Fix (4 parts):**
  1. Migration `supabase/migrations/20260714120000_project_draft_classification_nullable.sql` — drop NOT NULL on `projects.project_type/category/purpose` + add `projects_classification_required` CHECK (`status='draft' OR all three not null`), mirroring `properties_classification_required`. **Applied to the live DB** (verified: all three columns `is_nullable=YES`, constraint present).
  2. `src/lib/validators/project.ts` — `ProjectDraftSchema` makes the three fields `.nullable().optional()`; `ProjectSubmitSchema` re-requires them.
  3. `src/components/forms/ProjectForm.tsx` `buildPayload()` — send `undefined` for empty classification so the lenient draft schema accepts a partial Step-1 draft.
  4. `src/lib/actions/projects.ts` — `project_type/category/purpose: … ?? null` in create + update draft inserts.
- **Retest (live browser):** Step 1 (name/purpose/category) → **Continue now advances to Step 2 (Type & RERA)**; draft row persisted with `project_type=null, category=residential, purpose=sell, status=draft`. Step 2: complete 9-type set, honest RERA disclaimer ("No fake RERA badge is shown"), RERA-registered-without-number correctly blocked ("RERA number is required when RERA registered"). typecheck + eslint + `npm run build` all green. Property + Requirement wizards re-checked live — no analogous bug (property draft is title-lenient; requirement defaults purpose=buy/category=residential to valid enums).

## BUG-2026-07-13-03 — Search "Type" lists were a sample subset + Requirements type filter was dead (FIXED, retested)
- **Found:** Phase 2 follow-up (residual noted at the end of BUG-2026-07-13-02). The `PROPERTY_TYPES` option lists in `src/lib/search/config.ts` were hand-typed subsets of the canonical enums (e.g. `tenament`, `independent_house`, `commercial_building`, `single_room` were never offered), violating CLAUDE.md "DESIGN LISTS ARE SAMPLES — BUILD THE COMPLETE REAL SET". Separately, the Requirements tab's "Type" dropdown sent `?type=…` but the requirements branch in `public-search.ts` never read `params.type` → a fully dead filter (CLAUDE.md §NO DEAD UI).
- **Severity:** Medium (incomplete option set + one dead filter).
- **Fix:** `config.ts` now generates `PROPERTY_TYPES` from the canonical enums (`PROPERTY_TYPES_BY_CATEGORY`, `PROJECT_TYPES`, `REQUIREMENT_CATEGORIES`) via `toOptions()` + a `TYPE_LABELS` map (falls back to title-case so a new enum member is never hidden). Each tab shows the COMPLETE set for its scope category; values are canonical so they filter straight through (no aliases needed for new URLs; the old `normalizePropertyType`/`PROJECT_TYPE_FILTER` maps are kept for backward-compat with old bookmarked hyphenated URLs). `public-search.ts` requirements branch now applies `params.type` against `category`.
- **Retest (live browser):** Buy tab TYPE shows all 9 residential types; `/search?tab=buy&type=flat_apartment` → 1 result (the flat), Tenament excluded, active "Flat / Apartment" chip shown. Typecheck + eslint (changed files) green.

## BUG-2026-07-13-02 — Search property & project type filters matched nothing (FIXED, retested)
- **Found:** Phase 2 audit. `/search?type=flat` (and every property-type value) returned 0 results even when matching listings existed. Root cause: the search filter UI vocabulary in `src/lib/search/config.ts` (`flat`, `house`, `row-house`, `co-working`, hyphenated) never matches the canonical `property_type` enum stored in the DB (`flat_apartment`, `independent_house`, `row_house`, `co_working_space` — `src/lib/validators/property.ts`). `public-search.ts` passed the UI value straight into `.eq("property_type", …)`, so the filter silently excluded everything. The projects tab had the same class of bug (`project_type` = `apartment_project` vs config `residential-project`).
- **Severity:** High (a primary, always-visible search filter was completely dead — "control that does nothing", CLAUDE.md §NO DEAD UI).
- **Fix:** `src/lib/actions/public-search.ts` — added `normalizePropertyType()` (hyphen→underscore + explicit alias map to canonical enum) applied to the property `type` filter; added `PROJECT_TYPE_FILTER` mapping the projects tab's category/format intents to the correct column (`category` / `project_type`) + canonical value.
- **Retest (live, curl):** `/search?type=flat` → 1 (the `flat_apartment`); `/search?type=plot` → 0 (correct, none); `/search?tab=projects&type=residential-project` → 2 (both `apartment_project`/residential); non-existent categories → 0. Typecheck/lint/build green.
- **Residual (follow-up, not dead):** the offered type option lists are still a sample subset of the canonical enum (e.g. `tenament`/`independent_house` not offered as distinct options), and a couple of niche commercial/PG UI types map to the nearest canonical value. Offered values now all filter correctly; completeness of the option list is a separate enhancement.
