...
Unifies Leads/Requirements/Proposals/Site Visits/Direct Messages into one Instagram-DM-style inbox list, per user-approved UX spec (Instagram/WhatsApp-style unified messaging). No schema migration needed — `message_threads.lead_id`/`.proposal_id` already provided the linkage; this phase is a query + UI layer.
- **`listThreads()` (`src/lib/actions/messages.ts`):** new `resolveThreadContext()` joins each thread to its lead/proposal target (via `getTargetSummary`, now exported from `leads.ts`) and to any active `site_visits` row, producing a computed `source` (`lead`/`requirement`/`proposal`/`site_visit`/`message`), `contextTitle`/`contextCityText` (property/project/requirement title+city), `contextStatus`, and `isUrgent` (receiver, lead still `new`/`open`, <30 min old — surfaces the "5-minute reply" response-speed pattern). New `sourceFilter` param on `listThreads`.
- **`ThreadListClient.tsx`:** source badge icons + labels, source filter chip row (All/Leads/Requirement/Proposals/Site Visits/Messages), status chip (color-mapped), urgency highlight row + "Reply soon" chip, long-press (mobile, 500ms) / right-click (desktop) quick-status menu (Contacted/In Process/Closed → `updateLeadStage`).
- **Bug caught + fixed during live verification:** quick-status menu initially applied to every lead-linked thread, but for site-visit/proposal-sourced threads the displayed badge is driven by the site visit/proposal state machine, not `lead.crm_stage` — updating crm_stage there was silently invisible and misleading. Scoped the menu to `source === 'lead' || 'requirement'` only; site-visit/proposal threads keep their existing dedicated status flows.
- **Second bug caught + fixed:** `contextStatus` for lead-sourced threads initially read `lead.status` (18-value lifecycle enum) while the quick-status menu writes `lead.crm_stage` (10-value CRM stage) — so Contacted/In Process changes never appeared. Now reads `crm_stage` first, falling back to `status`.
- **Live-verified** as Test Owner (real dev data, real leads/site-visits/proposals): source filter chips correctly partition threads (Leads/Site Visits tested), right-click quick-status menu correctly opens only on lead/requirement threads and is absent on site-visit threads, "Contacted" stage change reflected live in the badge after fix. Responsive-checked at 320/375px — no overflow, chips scroll horizontally, no overlap. Gates: tsc/eslint/build all PASS.
- **Deferred to later phases (per locked plan, not started):** Phase 2 — in-thread sticky context card + in-chat number Request/Share wiring (contact.ts actions already exist, needs UI). Phase 3 — listing-share card in chat (needs a new message `metadata`/type column, a real schema change, treated as its own reviewed migration due to higher abuse-risk surface).

## 2026-07-15 (follow-up) — Unified Inbox: renamed to "Leads", old CRM-pipeline Leads page removed → PASS
User asked to close Phase 1's one pending item (tablet/desktop breakpoints untested) and rename "Messages" to "Leads" everywhere, explicitly approving removal of the old separate Leads list page (Kanban/table CRM pipeline).
- **Nav merge:** sidebar/drawer "Leads" (old `/dashboard/{role}/leads`) and "Messages" (`/dashboard/messages`) entries merged into one "Leads" entry per role in `src/components/dashboard/navConfig.ts`; broker's "Leads / CRM" and builder's "Project Leads" labels standardized to "Leads"; shared `getMobileTabs()` bottom-nav "Leads" tab repointed to `/dashboard/messages`; removed now-unused `MessageSquare` icon import.
- **Old pages removed, not left dangling:** `src/app/dashboard/{owner,broker,builder}/leads/page.tsx` replaced with `redirect("/dashboard/messages")` — the old Kanban/table UI is gone but the URLs still resolve (no broken links). "View Leads"/"View all" links on the 3 dashboard home pages repointed to `/dashboard/messages`.
- **Page title** renamed "Messages" → "Leads" (`src/app/dashboard/messages/page.tsx`).
- Confirmed `LeadListClient`/`LeadKanbanBoard`/`LeadsTable`/`LeadStageTabs`/`LeadStageBadge` are still used by the 3 dashboard home pages' "Recent Leads" widget — not deleted, not dead code.
- **Pending item closed:** tablet (768px) and desktop (1024px/1366px) breakpoints live-verified — single "Leads" nav entry, no overflow/overlap at any width.
- Gates: tsc/eslint/build all PASS. Live-verified as Test Owner: nav, home-page links, and direct URL redirects (including cross-role redirect, e.g. hitting `/dashboard/broker/leads` while logged in as Owner) all resolve correctly to the unified inbox.
...
- **Owner (deep):** overview/properties(draft-resume,tab counts)/requirements/leads+lead-detail(CRM stage, contact-hidden RLS, msg thread, site-visit accept/reject, follow-ups)/messages/site-visits/saved/recently-viewed/analytics(honest no-fake)/billing(real plan, honest empty invoices+payments)/verification(honest deferred)/notifications(real)/settings(honest provider states)/profile(masked mobile). All real data, no leaks.
- **Broker:** overview(verified badge, Proposals=1 matches owner's "1 received")/proposals(sent list, honest received-empty, open-requirements feed, Send-Proposal modal, **duplicate correctly blocked**)/leads-CRM Kanban(renders)/public-profile(honest Setup-Required).
- **Builder (ads skipped):** projects(2 real, Units/Construction/Edit)/unit-inventory(wing gen 5×3=15, unit list, edit, bulk, filters; mobile table→cards)/agents-team(invited agent + granular permission toggles + invite)/construction(stage stepper, milestone post/delete, honest "nothing simulated").
- **Shells/security/responsive:** all 3 role sidebars correct + NO admin bottom-nav + locked Home·Search·Post·Leads·Profile; wrong-role route → "Access Denied" (not login); object ownership enforced in server actions + RLS; 390px table→cards + no horizontal overflow. Gates: tsc PASS · lint PASS · build PASS.
- **Honestly deferred / NOT built (not fake — clearly labelled):** Profile Edit ("Coming Soon"), Verification submit ("not open yet"), Role Change, Data Export, Account Deletion+cancel, user-facing Support tickets (static contact page only), Broker+Builder Public Profile/Microsite ("Setup Required"). Full team-permission enforcement matrix (active/invited/suspended/scoped agents) not exhaustively driven (no seeded agent logins). Billing "Active listings" usage counter reads 0 vs 2 live (event-incremented; acceptable while enforcement off).

## 2026-07-14 — Banner Ads system + home hero swap (ported from original project, user-approved) → PASS
Replaced the whole "Ads / Promotions" (ad_campaigns) concept with the original My Gujarat Property project's **Banner Ads** system, and swapped the home hero for that project's `SearchHero`. Both wired end-to-end and live-verified.
- **Removed (user-approved full replacement):** `ad_campaigns` table (dropped in new migration) + `src/lib/actions/ads.ts`, `src/lib/ads/`, `AdCreateForm.tsx`, `AdDetailClient.tsx`, `adStatus.ts`, `dashboard/builder/ads/[id]/page.tsx`, migration `20260714152000_ad_campaigns.sql`.
- **New migration (applied to live dev DB):** `20260714160000_banner_ads.sql` — `banner_ads` + `banner_ad_targets` tables, RLS (anon reads ONLY approved+active+paid+in-window ads; advertiser reads own via profiles↔auth.uid join; all writes service-role after server role check), public `banner-ads` storage bucket (folder = auth uid). Adapted to this project's `profiles`/`projects`/`public_projects_view`/`project_units` + `mgp_set_updated_at()`.
- **New lib:** `src/lib/banner/{config,types,media,queries,actions}.ts` — builder-eligible only; promotes a PUBLISHED project; card (name/price/BHK/location) built from the project, never advertiser free-text; approval-required (admin review); payment `not_required` (Razorpay ads NOT wired → never faked); moderation actions logged to `admin_audit_logs`.
- **New components:** `src/components/banner/*` (BannerAdForm + live Top-picks preview, BannerImageUploader real R2 upload, BannerCarousel homepage Top-picks, BannerListActions, BannerModerationActions, BannerStatusBadge, SponsoredLabel).
- **Pages:** rebuilt `/dashboard/builder/ads` (list) + `/new` + `/[id]/edit` in DashboardShellV2; new `/admin/moderation/banner-ads` queue (+ card on moderation index); builder nav label → "Banner Ads".
- **Home:** `src/components/public/HomeHeroSearch.tsx` replaced with the original project's hero (trust pill, gradient blobs, tabbed search card over the FULL real SEARCH_TABS/PROPERTY_TYPES, quick-city chips, trust badges) — correctly rewired to `/search?tab=…&min_price=…` (the current search page reads `tab`, not `purpose`; the old component was mis-wired). Added `<BannerCarousel>` (renders only real approved ads; hides when none).
- **Checks:** `tsc --noEmit` PASS · `npm run lint` PASS (0/0) · `npm run build` PASS. Live-verified: hero search routes correctly; builder create form pulls a REAL published project, Save-draft persists + shows in list; admin queue reads real data with logged moderation controls. Test draft removed afterward (0 rows left).

## 2026-07-14 — Phase 3: deep builder features (Team / Ads / Construction / Project Analytics / Microsite) → PASS
Built the last remaining §28 deep screens with REAL backends + honest provider-gated states. CRM Kanban (B7-S10) was already built. Phase 3 screen inventory is now functionally complete.
- **New migrations (applied to live DB):** `20260714150000_project_construction_updates.sql`, `20260714151000_builder_team_members.sql`, `20260714152000_ad_campaigns.sql` — idempotent, RLS owner-manages-own via `mgp_get_my_profile_id()`, public read for construction updates on published projects. Applied with new helper `scripts/apply-migration.mjs`.
- **Project Analytics (B8-S19):** real project/unit/lead/visit counts + honest "views not tracked" panel.
- **Construction Progress (B8-S24):** builder-posted milestones (`construction.ts`); posting syncs `projects.construction_status`. Live-verified persist + sync.
- **Team/Invite/Permissions (B8-S12/13/14):** real roster + jsonb permission flags + single-use invite token + pre-auth accept route `/team/invite`. Email delivery Setup-Required → invite link surfaced (no fake "sent"). Live-verified invite + permission persistence.
- **Ads (B8-S15-18):** real campaign CRUD (draft→pending_review→cancel). Payment provider-gated — stays unpaid, never auto-activates (honest Setup Required, no fake activation); analytics honest "not tracked". Live-verified create→submit-for-review.
- **Company Microsite (B8-S23):** real management screen — live `/builder/[slug]` URL + published-project list + honest not-published state.
- Bug fixed: `AD_TYPES` exported from a `"use server"` file broke the build → moved to `src/lib/ads/config.ts` (same class as the earlier LANGUAGE_OPTIONS fix).
- Gates: typecheck PASS · eslint PASS · `npm run build` PASS (7 new routes). Console clean. **Result: PASS.**

## 2026-07-14 — Phase 3: Analytics + Settings + Recently Viewed (real backends) → PASS; all dead `#` nav eliminated
Built the remaining dead/absent dashboard screens with real data + honest empty states. **No `href:"#"` dead nav remains** in navConfig.ts.
- **Analytics** (owner B6-S13, broker B7-S15): shared `AnalyticsOverview` — real live counts (listings/leads/site-visits) + honest "Views & impressions — Not tracked yet" panel (no view-tracking table; never a fake view count).
- **Settings** (owner B6-S22, broker B7-S22, builder B8-S27): `SettingsClient` — real language preference persisted to `profiles.language_preference` via new `updateLanguagePreference` action (own-row RLS, value-checked); honest notification-channel status (in-app On; email/SMS/WhatsApp Setup Required, not fake toggles); real account read-out + quick links.
- **Recently Viewed** (owner B6-S12, broker B7-S12): real `recently_viewed_items` history via existing `listRecentlyViewed()` + Clear history; honest empty/"No longer available" states.
- **navConfig.ts**: owner Analytics/Settings/Recently-Viewed now real (were `#`); added Recently Viewed + Analytics + Settings to broker nav; added Settings to builder nav.
- **Bug fixed**: `LANGUAGE_OPTIONS` exported from a `"use server"` file became a server reference and broke the client `.map()` (error boundary). Moved to plain module `src/lib/settings/language.ts`.
- Live-verified per role (real OTP login): analytics real counts + honest panel; Settings language save persisted to DB (`gu` owner, `hi` builder) and reset to `en`; recently-viewed real items; fresh-tab console clean; 0 dead sidebar links. Gates: typecheck PASS · eslint PASS · `npm run build` PASS (8 new routes). **Result: PASS.**

## 2026-07-14 — Phase 3: Site Visits wired for Owner + Broker + Builder (B6-S08/S09, B7-S13, B8-S11) → PASS
Wired the **Site Visits** screen for all three public roles and verified each live with dev dummy data.
- Shared enrichment helper `src/lib/dashboard/site-visit-rows.ts` (`buildSiteVisitRows`) + shared `src/components/dashboard/SiteVisitsClient.tsx`; three thin role pages `src/app/dashboard/{owner,broker,builder}/site-visits/page.tsx`. Owner page refactored to use the shared helper. Data via `listMySiteVisits()` (RLS: requester OR host), enriched with **real** property/project titles + counterpart names — no fabricated fields. Accept/Reject/Reschedule/Cancel call the real site-visit server actions (participant + status-transition enforced) — no fake data, no dead controls.
- `navConfig.ts`: enabled owner Site Visits link (was `#`); **added** Site Visits nav items to broker + builder shells (they had none).
- Dev dummy data: `scripts/seed-dev-site-visits.mjs` (+ `--clean`) seeds 5 varied `site_visits` for Test Owner as host + broker/other-owner/builder counterparts; tracked in `scripts/dev-site-visits.fixture.json`, **remove before production**.
- Live-verified (real OTP login per role): Owner (host) — 5 upcoming + 2 past, **Accept drove requested→accepted live and persisted**, Reschedule bottom-sheet + scroll-lock/restore, mobile @375 no overflow, locked bottom nav. Broker + Builder (requesters) — visits shown as "Hosted by Test Owner" with **no Accept/Reject** (role-correct host gating). No console errors.
- Gates: typecheck PASS · eslint PASS · `npm run build` PASS (all 3 routes emitted). **Result: PASS.**
- **Honest remaining Phase 3 gaps (NOT done):** Analytics + Settings (all roles) kept honestly-disabled — no real backend, and faking counts would violate the no-fake-analytics rule; Broker CRM Kanban, and several Builder-specific screens (Ad Create/Detail/Analytics, Project Analytics, Team Invite/Permissions, Construction Progress, Company Microsite) are partial/absent pending real backends.
