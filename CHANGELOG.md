...
## 2026-07-10 — Deep-check: Home / Search / Login vs finished wireframes (bug-fix pass)

Audited the three public screens side-by-side against the Batch 3 (home + search) and Batch 2 (auth) design source in the browser at 390/1366px, and fixed the mismatches found:
- **Home — Role CTA band** (`HomeRoleCards.tsx`): rebuilt to Batch 3 Screen 1 — full-width brand `#0F6B5C` band, centered "List with us — it's free" + "Reach genuine buyers and tenants across Gujarat directly.", 3 role cards (Owner/Broker/Builder) with icon squares, design copy, and white "Post … for Free" CTAs (wired: guest → auth popup, logged-in → role dashboard). Replaces the previous off-design dark-green `#0E3B2E` split-panel version.
- **Home — Trust section** (`HomeTrust.tsx`): rebuilt to Batch 3 "Why My Gujarat Property" — 3 white cards (Verified listings / Direct owner contact / Local Gujarat expertise) with exact design copy. Replaces the off-design "Built for trust…" split panel.
- **Home — How it works** (`HomeHowItWorks.tsx`): rebuilt to Batch 3 — circular brand-soft icon steps "1 · Search / 2 · Connect / 3 · Close" joined by chevrons on desktop, stacked on mobile. Replaces the numbered-card variant.
- **Home — Featured projects** (`HomeFeaturedProjects.tsx`): rebuilt from the off-design dark cards to Batch 3 white project cards — price range, name, location, RERA pill (only when `rera_registered` is true — wording kept "RERA Registered", never a fake "Verified"), footer initials avatar + project-type/construction-status line (no fake builder/units data).
- **Search — result card raw enums** (`PropertyResultCard.tsx`): area chip showed raw `1450sq_ft` and type chip raw `Flat Apartment`/`Tenament` casing — now uses the existing `formatArea()`/`labelize()` helpers ("1,450 sq ft", "Flat / Apartment"-style labels).
- **Kept as approved deviations (NOT bugs):** desktop header without primary-nav/mega-menu row (explicitly removed in commit `6b2d59a` on 2026-07-09), graphite footer + single dark Login/Register button (per the newer "for header sidebar mobile" design, CLAUDE.md §40 override), widened `max-w-7xl` home containers (same commit).
- **Verified live:** home (guest + logged-in Owner incl. Recently-viewed strip) at 390/1366 — no horizontal scroll, no console errors; search filters write real query params (`?bhk=1`) and honest empty state renders; full mobile-OTP login flow (unregistered-number branch + existing-user OTP + auto-submit) completes to `/dashboard/owner`.
- Checks: lint PASS, `tsc --noEmit` PASS, `npm run build` PASS.
