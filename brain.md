...
## RESUME GUIDE — 2026-07-15 · Unified Inbox Phase 1 (source badges/filters/quick-status) → PASS
- User asked for an Instagram-DM-style unified inbox (public-role dashboards): Leads/Requirements/Proposals/Site Visits/Direct Messages all in one list, with source badges, property/project context shown like a WhatsApp link-preview, in-chat number request/share, listing-share cards, and a mobile long-press / desktop right-click quick-status menu. Locked a 3-phase build order with the user before starting (not all in one shot):
  - **Phase 1 (done, this entry):** unified list — source badges, thumbnails/context title, status chips, source filter chips, urgency highlight for unreplied new leads, quick-status menu.
  - **Phase 2 (not started):** in-thread sticky context card + wiring the in-chat "Request Number"/"Share Number" buttons to the already-existing `src/lib/actions/contact.ts` (`requestContactReveal`/`approveContactReveal`) — UI-only work, no new schema expected.
  - **Phase 3 (not started):** share-a-listing-in-chat card. Needs a real schema change (a `message_type`/`metadata` column on `messages` — currently text-only body) and stronger abuse controls (ownership check, approved-only, daily rate limit, report option) — deliberately scoped as its own reviewed migration, not bundled into Phase 1/2.
- **Why no migration was needed for Phase 1:** `message_threads` already had `lead_id`/`proposal_id` FKs and a `thread_type` enum anticipating `requirement`/`site_visit` values — the unification is a query+UI layer on existing data, not a schema rewrite.
- **Code changed:** `src/lib/actions/leads.ts` (`getTargetSummary` now exported, was private), `src/lib/actions/messages.ts` (`resolveThreadContext()` + `ThreadSource` type + `sourceFilter` param on `listThreads()`), `src/components/messages/ThreadListClient.tsx` (full UI rewrite — source chips/badges, status chip, long-press/right-click quick-status calling `updateLeadStage`).
- **Two real bugs found and fixed during live browser verification** (not just typecheck/build — see CHANGELOG 2026-07-15 entry for detail): (1) quick-status menu was reachable on site-visit/proposal-sourced threads where the displayed badge doesn't come from `lead.crm_stage`, making the action silently no-op from the user's POV — scoped the menu to `lead`/`requirement` sources only; (2) the badge itself read `lead.status` instead of `lead.crm_stage`, so even valid quick-status changes (Contacted/In Process) never visibly updated — fixed to read `crm_stage` first.
- **Live-verified** as Test Owner (real dev data — Test Broker requirement-inquiry thread, two Test Builder/other site-visit threads): All/Leads/Site Visits filter chips correctly partition; right-click menu correctly absent on site-visit threads, present on the requirement thread; "Contacted" click updated the visible chip after the fix; responsive-checked 320px/375px (mobile), no overflow. Gates: tsc/eslint/build all PASS.
- **2026-07-15 follow-up — nav rename + old Leads page removal (user-approved), Phase 1 pending item closed → PASS:** User asked to fix Phase 1's one pending item and rename "Messages" → "Leads", explicitly approving removal of the old separate CRM-pipeline Leads page (`src/app/dashboard/{owner,broker,builder}/leads/page.tsx`, using `LeadKanbanBoard`/`LeadsTable`). Those 3 pages now just `redirect("/dashboard/messages")` (no broken links — old bookmarked URLs still resolve, the old Kanban/table UI itself is gone). Merged the old "Leads" and "Messages" sidebar/drawer nav entries into a single "Leads" item (all 3 roles) in `src/components/dashboard/navConfig.ts`, repointed the shared `getMobileTabs()` bottom-nav "Leads" tab, repointed "View Leads"/"View all" home-page links, renamed the page title. Confirmed via grep that `LeadListClient`/`LeadKanbanBoard`/`LeadsTable`/`LeadStageTabs`/`LeadStageBadge` are still used by the 3 dashboard-home "Recent Leads" widgets — kept, not dead code. Closed the pending tablet/desktop-breakpoint check live (768/1024/1366px — single "Leads" entry, no overflow). Gates: tsc/eslint/build all PASS. **Phase 1 now has zero pending issues.**
- **Next:** do not start Phase 2/3 without the user's go-ahead — same one-phase-at-a-time cadence as the Batch 9/10 work below.

---

## RESUME GUIDE — 2026-07-14 · Phase 4 Batch 9 (Shared Lead Ops) → PASS
- User is running Phase 4 one batch at a time (Batches 9–13: Shared Lead Ops, Billing, Admin Management/Moderation, Admin Billing, Ads/Notifications/System). Started with **Batch 9**. Note: the Phase 4 prompt's generic "Ads" is superseded by the already-built **Banner Ads** system (banner-ads, not ad_campaigns) — see the 2026-07-14 Banner Ads resume note below.
- Survey found most of Batch 9's backend already existed from Prompt 08 (leads, contact reveal, proposals, messages, site visits, shared `LeadDetailClient.tsx` workspace). FEATURE_REGISTRY's old CRM/MSG rows were stale relative to actual code. Closed the **genuinely missing** pieces only: Close/Lost w/ structured reason, duplicate-lead detect+dismiss (merge deferred — see why below), standalone Proposal detail page with embedded thread, Report Thread, site-visit reject-reason/duplicate-guard/outcome-marking/feedback/dispute.
- **Migration applied:** `supabase/migrations/20260714170000_batch9_lead_ops_extensions.sql` — `leads.close_reason`/`.close_reason_detail`, `site_visits.reject_reason`/`.feedback_rating`/`.feedback_comment`/`.feedback_submitted_by`/`.feedback_submitted_at`/`.dispute_reason`/`.disputed_by`/`.disputed_at`, new `lead_duplicate_flags` table (RLS participant-scoped), widened `notifications` type check constraint.
- **New/changed code:** `src/lib/actions/leads.ts` (`closeLead`, `getPossibleDuplicateLeads`, `dismissDuplicateLeadFlag`), `src/lib/leads/close-reasons.ts` (moved out of the `"use server"` file — same class of bug as the `LANGUAGE_OPTIONS`/`AD_TYPES` fix from earlier sessions: never export a non-async const from a `"use server"` file), `src/lib/actions/site-visits.ts` (reject-reason param, duplicate-request guard, `submitSiteVisitFeedback`, `disputeSiteVisitOutcome`), `src/lib/actions/messages.ts` (`reportThread`, reuses existing `user_reports` table), `src/lib/permissions/communication-permissions.ts` (fixed `SITE_VISIT_TRANSITIONS`: `accepted`→`completed`/`no_show` was missing — a host who accepted without setting a schedule had no way to ever mark a visit complete), `src/components/leads/LeadDetailClient.tsx` (all the above wired in), new `/dashboard/proposals/[id]` page + `ProposalDetailClient.tsx`, `ProposalListClient.tsx` now links to it.
- **Merge (duplicate leads) intentionally deferred**, not faked: safely combining notes/proposals/site-visits/timeline across two lead rows needs a real transactional merge routine — out of scope for this pass. Detect+dismiss is real and live-verified; merge stays `DEFERRED_TRACKED`.
- **Live-verified as Test Owner** against real (non-seeded) Test Broker/Builder data, confirmed via page reload each time (not just optimistic client state): duplicate-flag detected + Compare link + Dismiss; site-visit Reject blocked without a reason then succeeded with reason persisted (confirmed on Site Visits "Past" tab as `Rejected`); Mark Completed/feedback (4/5 + comment) persisted; Dispute persisted (fixed a UI gap where the dispute control disappeared after feedback was submitted — now stays available until actually disputed); Close Lead with reason `went_with_another` transitioned Negotiation→Closed and disabled further stage edits; Report Thread → "Reported" state (guarded against a second report server-side); Proposal detail page auto "sent"→"viewed", embedded thread message sent, Shortlist transition. One HMR false-negative during testing (page reloaded mid-click due to concurrent file edits) turned out to be a test artifact, not a real bug — resolved by re-testing after edits stopped.
- Gates: `tsc --noEmit` PASS · eslint (changed files) PASS · `npm run build` PASS (`/dashboard/proposals/[id]` route emitted, 0 new errors).
- **2026-07-14 follow-up — pending issues closed:** user asked to fix the three pending gaps. Added migration `20260714180000_batch9_merge_archive_report_status.sql`: `mgp_merge_leads()` SQL function (SECURITY DEFINER, transactional — one plpgsql function body, rolls back atomically on any failure) doing a real duplicate-lead merge (notes/followups/contact_requests/proposals/site_visits/crm_events reassigned, message threads consolidated, duplicate archived with `close_reason='duplicate'`, `lead_merged` crm_event logged); `mergeDuplicateLeads()` action + "Merge" button with inline confirm. Also added `message_threads.participant_a_archived`/`.participant_b_archived` + `archiveThread()`, rewrote `listThreads()` to take `(filter, search, page, limit)`, rewrote `ThreadListClient.tsx` as a self-fetching component with All/Unread/Archived tabs + debounced search + pagination. Added `getThreadReportStatus()` so the "Reported" badge survives a reload. **Live-verified the merge via a direct RPC test against real data** (not synthetic) — confirmed 4 messages + 2 site visits (including the feedback/dispute data from the earlier test session) correctly consolidated onto the surviving lead, full timeline preserved. Messages archive/search live-verified via UI. Test script `scripts/test-merge-batch9.mjs` was throwaway and deleted after use. Gates: tsc/eslint/build all PASS.
- **Next:** user picks the next Batch (10 Billing / 11 Admin Management / 12 Admin Billing / 13 Ads+Notifications+System) — do not start it without asking, per the established one-batch-at-a-time cadence.

---

## RESUME GUIDE — 2026-07-14 · Banner Ads system + home hero swap (PASS, user-approved port)
- User approved porting the ORIGINAL project (`C:\Users\RAJAN\my-gujarat-property`) Banner Ads dashboard + its home `SearchHero`, replacing the `ad_campaigns` "Ads / Promotions" concept entirely.
- **DB:** migration `20260714160000_banner_ads.sql` APPLIED to dev DB — `banner_ads` + `banner_ad_targets` + public `banner-ads` bucket; drops `ad_campaigns`. RLS: anon reads only publicly-displayable ads; advertiser reads own via `profiles.auth_user_id = auth.uid()`; writes via service role after server role check. `approved_by` = staff_profiles.id (no FK).
- **Model:** builder-only; a banner promotes ONE of the builder's PUBLISHED `projects`; card built from `public_projects_view` + `project_units` (never advertiser free-text); approval-required; `payment_status='not_required'` (Razorpay ads NOT wired — never faked); moderation logged to `admin_audit_logs`.
- **Code:** `src/lib/banner/*`, `src/components/banner/*`, pages `/dashboard/builder/ads` (+`/new`,`/[id]/edit`), `/admin/moderation/banner-ads` (+index card). Home `HomeHeroSearch.tsx` = ported hero, rewired to `/search?tab=…` (search page reads `tab`, NOT `purpose` — the earlier hero was mis-wired). `<BannerCarousel>` added to home (renders only real approved ads).
- **Verified live:** builder create form pulls real published project "Test Grand Vista Towers"; Save-draft → DB → list works; admin All-queue reads real data; hero routes `?tab=rent&city=…`. tsc/lint/build all PASS. Test draft deleted afterward.
- **Note:** removed old ad files (`lib/actions/ads.ts`, `lib/ads/`, `AdCreateForm/AdDetailClient/adStatus`, old `[id]/page.tsx`, `20260714152000_ad_campaigns.sql`).
...
## RESUME NOTE — 2026-07-14 · Phase 3 · Deep builder features (Team/Ads/Construction/Project-Analytics/Microsite) → PASS
Built the last remaining §28 deep screens with REAL backends (3 new migrations, applied to live DB) + honest provider-gated states. CRM Kanban (B7-S10) was found already-built. **Phase 3 screen inventory is now functionally complete** (provider-gated bits honestly SETUP_REQUIRED, never faked).
- **Migrations applied to live DB** (via `scripts/apply-migration.mjs` — direct pg, `db.<ref>.supabase.co:5432`): `20260714150000_project_construction_updates.sql`, `20260714151000_builder_team_members.sql`, `20260714152000_ad_campaigns.sql`. All idempotent, RLS via `mgp_get_my_profile_id()` (owner-manages-own) + public read for construction updates on published projects.
- **Project Analytics (B8-S19)** — `BuilderAnalyticsOverview` + `/dashboard/builder/analytics` (+ builder nav). Real projects/units(by availability)/leads/visits counts + honest "views not tracked" panel. Live: 2 projects/12 units/2 upcoming.
- **Construction Progress (B8-S24)** — `/dashboard/builder/projects/[id]/construction` + `ConstructionProgressClient` + `src/lib/actions/construction.ts` (list/add/delete, owner-checked) + `src/lib/projects/construction.ts`. Posting a milestone also syncs `projects.construction_status` (drives public timeline). Live-verified: posted "5th floor slab 45% under_construction" → persisted + project stage synced. "Construction" link added to project rows.
- **Team/Agents/Invite/Permissions (B8-S12/13/14)** — `/dashboard/builder/agents` (real roster, replaced placeholder) + `TeamClient` + `src/lib/actions/team.ts` + `src/lib/team/permissions.ts` + pre-auth accept route `/team/invite` (`TeamInviteAccept`). Invite creates a real row + single-use token; **email delivery is Setup-Required so the invite link is surfaced to share manually (no fake "sent")**. Permission flags stored in jsonb. Live-verified: invite "Ravi Sales" created + link shown + permission toggles persisted (manage_leads/projects/units).
- **Ads (B8-S15/16/17/18)** — `/dashboard/builder/ads` (list, replaced placeholder) + `/ads/new` (`AdCreateForm`) + `/ads/[id]` (`AdDetailClient`) + `src/lib/actions/ads.ts` + `src/lib/ads/config.ts` + `adStatus.ts`. Real campaign CRUD (draft→pending_review→cancel). **Payment is provider-gated: campaigns stay `payment_status='unpaid'` and never auto-activate — honest "Setup Required" (Razorpay ads not wired), NO fake activation.** Analytics = honest "not tracked". Live-verified: create ₹5000 draft → submit-for-review → status=pending_review, payment stayed unpaid.
- **Company Microsite (B8-S23)** — `/dashboard/builder/public-profile` upgraded from placeholder to real management screen: live public URL (`/builder/[slug]`) + real published-project list + honest not-published state.
- **Bug fixed (2nd occurrence of the same class):** `AD_TYPES` const was exported from `"use server"` ads.ts → build error "use server file can only export async functions". Moved to `src/lib/ads/config.ts`. (Same fix pattern as `LANGUAGE_OPTIONS`.)
- Gates: `tsc` PASS · eslint PASS · `npm run build` PASS (7 new routes emitted). Console clean. **Result: PASS.**
- **Dev test rows left from verification** (test builder acct, dev-only): 1 construction update, 1 team member ("Ravi Sales", invited), 1 ad campaign (draft/pending, mistitled "Rajkot" due to a test-input selector clash — cosmetic). Remove before production if desired.

---

## RESUME NOTE — 2026-07-14 · Phase 3 · Analytics + Settings + Recently Viewed → PASS (dead `#` nav eliminated)
Built the remaining dead/absent dashboard screens with **real backends + honest empty states** (no fabricated data). After this, **zero `href:"#"` dead nav remains** anywhere in `navConfig.ts`.
- **Analytics** (B6-S13 owner, B7-S15 broker) — shared async server component `src/components/dashboard/AnalyticsOverview.tsx`: real live counts (total listings via `getMyProperties`, total/open/new leads via `getLeadCounts`, upcoming/completed visits via `listMySiteVisits`) + an **honest "Views & impressions — Not tracked yet"** panel (no view-tracking table exists; we never show a fake view count). Pages: `src/app/dashboard/{owner,broker}/analytics/page.tsx`.
- **Settings** (B6-S22 owner, B7-S22 broker, B8-S27 builder) — `src/components/dashboard/SettingsClient.tsx`: **real** language preference persisted to `profiles.language_preference` (`en/gu/hi`) via new action `src/lib/actions/settings.ts#updateLanguagePreference` (RLS: own row only, value-checked); honest notification-channel status (in-app "On"; email/SMS/WhatsApp "Setup Required" — not fake toggles); real account read-out + real quick links. Pages: `src/app/dashboard/{owner,broker,builder}/settings/page.tsx`.
- **Recently Viewed** (B6-S12 owner, B7-S12 broker) — real history from `recently_viewed_items` via existing `listRecentlyViewed()` + `ClearHistoryButton`; honest empty state; unresolved items show "No longer available". Pages: `src/app/dashboard/{owner,broker}/recently-viewed/page.tsx`.
- **navConfig.ts:** owner sidebar+drawer Analytics/Settings/Recently-Viewed now real; **added** Recently Viewed + Analytics + Settings to broker nav; **added** Settings to builder nav. No `#` left.
- **BUG fixed during verify:** `LANGUAGE_OPTIONS` was first exported from the `"use server"` settings action file → Next turned it into a server reference and the client `.map()` threw ("LANGUAGE_OPTIONS.map is not a function", error boundary). Fix: moved the constant to a plain module `src/lib/settings/language.ts`, imported by both the action and the client. (Rule learned: never export non-async consts from a `"use server"` file.)
- **Live-verified (real OTP login):** owner Analytics shows real 7 listings / 4 leads / 4 upcoming / 1 completed + honest views panel; owner Settings language save **persisted `gu` to DB** and UI reflected it; owner Recently Viewed shows 3 real items; builder Settings language save **persisted `hi` to DB**; fresh-tab console **clean, no errors**; owner desktop sidebar has **0 dead links**. Gates: tsc PASS · eslint PASS · `npm run build` PASS (8 new routes emitted).
- **Note:** verification restarted the dev server (killed the stale PID 5712 from another chat; cleared `.next/dev`). Test-account language prefs reset back to `en` after testing.

---

## RESUME NOTE — 2026-07-14 · Phase 3 · Site Visits (B6-S08/S09, B7-S13, B8-S11) → PASS (all 3 roles)
Wired the **Site Visits** screen for Owner, Broker AND Builder and verified all three live with dev dummy data.
- **Shared enrichment helper** `src/lib/dashboard/site-visit-rows.ts` (`buildSiteVisitRows(profileId)`) — loads `listMySiteVisits()` (RLS: requester OR host) and enriches each row with the **real** property/project title + counterpart name (no fabricated fields). Used by all three role pages.
- **Routes:** `src/app/dashboard/{owner,broker,builder}/site-visits/page.tsx`, each mounting the shared `SiteVisitsClient` in its own role shell. Owner page refactored to use the shared helper (was inline). `navConfig.ts`: owner Site Visits link enabled (was `#`); **added** Site Visits nav items to broker (after Messages) and builder (after Messages) — they had none before. Actions call the real `respondSiteVisit`/`rescheduleSiteVisit`/`cancelSiteVisit` (participant + status-transition enforced).
- **Dev dummy data:** `scripts/seed-dev-site-visits.mjs` seeds 5 varied `site_visits` for Test Owner as host + counterparts broker/other-owner/builder, covering requested/rescheduled/completed/cancelled(+reason) + an owner-as-requester project visit. Tracked in `scripts/dev-site-visits.fixture.json`; **remove before production** with `--clean`. See [[project-sitevisits-dev-fixture]].
- **Live-verified (real OTP login each role):** Owner (host) sees 5 upcoming + 2 past, **Accept drove requested→accepted live and persisted**, Reschedule bottom-sheet + scroll-lock/restore, mobile @375 no overflow, locked bottom nav. Broker (requester) sees its visits as "Hosted by Test Owner" with **no Accept/Reject** (correct — only host gets those). Builder (requester) sees 2 upcoming likewise host-gated. Nav item renders + active in each shell. No console errors.
- Gates: `tsc --noEmit` PASS · eslint PASS · `npm run build` PASS (all 3 `/dashboard/*/site-visits` routes emitted). **Site Visits = PASS across all roles.**
- **Remaining Phase 3 §28 gaps (NOT done — honest):** Analytics (B6-S13/B7-S15/B8-S19) + Settings (B6-S22/B7-S22/B8-S27) are still honestly-disabled `#` nav (kept disabled, not faked — no real analytics/settings backend yet; building fake counts would violate the no-fake-analytics rule). Broker CRM Kanban (B7-S10), and several Builder-specific screens (Ad Create/Detail/Analytics B8-S16-18, Project Analytics B8-S19, Team Invite/Permissions B8-S13/14, Construction Progress B8-S24, Company Microsite B8-S23) are partial/absent. These need real backends before they can be marked done.

---

## RESUME NOTE — 2026-07-13 · Phase 2 (Public Experience) IN_PROGRESS
Phase 2 is **~90% pre-built** (home, /search, /property/[slug], /project/[slug], /broker /builder /owner public profiles, all 3 wizards + [id]/edit, projects/[id]/units, /compare, FullscreenGallery, ReportModal, ClaimProfileCard). Do NOT rewrite these — Phase 2 is a **gap-close + per-screen 1:1 audit** task, not greenfield.
- **DONE this session:** the one true gap — public **Requirement Detail (B4-S03)** at `/requirement/[slug]` (`getRequirementDetail` action + `RequirementProposePanel` + card link). Enforces the locked verified-broker/builder-only audience (full view + propose) vs locked teaser for everyone else; `noindex` + sitemap-excluded. Live-verified all 4 audience states incl. proposal submit + duplicate-block. typecheck/lint/build all PASS.
- **Dev fixtures added (dev-only, flagged):** 1 published requirement owned by testowner; testbroker `verification_status` set to `verified` (was `not_started`) so the eligible audience can be tested — revert if it interferes with other verification.
- **Audit done this session (functional/security/responsive):** guest route sweep of ALL public routes → 200s, no crashes, **no contact/secret leaks**, correct 404s (owner-profile privacy-404 is intentional). Verified live: property detail (contact masked, reveal self/login notes, self-enquiry server-blocked), owner public profile, Compare (tray+localStorage+/compare render, noindex), 9-step property wizard (draft-resume with real Gujarati draft, step rail, sticky footer, honest maps fallback), Requirement Detail (4 states).
- **BUG-2026-07-13-02 fixed (High):** property + project search **type filters were dead** (UI vocab ≠ canonical enum). Fix = `normalizePropertyType()` + `PROJECT_TYPE_FILTER` in `src/lib/actions/public-search.ts`. Retested green.
- **Still pending for a full Phase 2 PASS:** deep 1:1 audit of project-detail internals (tabs/units/RERA), Unit Inventory (E), Claim (M), Fullscreen Gallery (O — couldn't exercise, no-media fixture), Report rate-limit (N); + the search type-option lists should be regenerated from the canonical enum for completeness (offered values now filter correctly).

---

## RESUME NOTE — 2026-07-14 · Phase 2 FULL manual verification → PASS (1 critical fix)
Drove the full Phase 2 verification matrix live and fixed every issue found.
- **CRITICAL fix — BUG-2026-07-14-01: Project wizard was dead at Step 1.** `projects.project_type/category/purpose` were still NOT NULL (Batch 5 draft-relaxation applied to properties only, never projects) + `ProjectDraftSchema` required them → Step-1 `createProjectDraft` always failed. Fixed: new migration `20260714120000_project_draft_classification_nullable.sql` (**applied to live DB** via pg direct connection — drop NOT NULL + `projects_classification_required` conditional CHECK), lenient `ProjectDraftSchema` + re-required `ProjectSubmitSchema`, `ProjectForm.buildPayload` empty→undefined, `projects.ts` action `?? null`. Live-retested: wizard advances to Step 2, draft persists with `project_type=null`, RERA validation works.
- **All three wizards live-verified** advancing step 1→2 with complete canonical option sets + autosave (property: 8 interactive steps + Submitted = 9 per design; project: 9; requirement: 7). Property draft-resume card works. Requirement/property have no analogous bug (defaults/lenient draft).
- **Live-verified PASS also:** guest privacy (no phone/email leak in server HTML), SEO/robots/sitemap/canonical/JSON-LD/noindex, tombstone+private-profile 404, wrong-role→Access Denied (not login), responsive no-overflow @320/390/430/768/1024/1280, Unit Inventory bulk mutation, Claim+Report submit+dup/rate-limit, gallery real images.
- Gates after fix: `tsc --noEmit` PASS · eslint PASS · `npm run build` PASS. **Phase 2 = PASS.**
- Dev cleanup still pending before production: gallery `dev-fixture-*` images ([[project-gallery-dev-fixture]]).
- Note: `pg` was installed with `--no-save` to apply the migration; harmless (not in package.json). Migration file is committed under supabase/migrations.

---

## RESUME NOTE — 2026-07-13 (session 2) · Phase 2 pending items CLOSED
Cleared all six pending Phase 2 items from the note above.
- **Pending 6 (search type lists) — FIXED + live-verified.** `src/lib/search/config.ts` `PROPERTY_TYPES` is now generated 1:1 from the canonical enums (`PROPERTY_TYPES_BY_CATEGORY`, `PROJECT_TYPES`, `REQUIREMENT_CATEGORIES`) via `toOptions()` — the COMPLETE set per tab, values already canonical so each option maps straight to a live column filter (no sample subset, no dead options). Also wired the previously-DEAD requirements "Type" filter: the requirements branch in `public-search.ts` now applies `params.type` against `category`. Live: Buy tab shows all 9 residential types; `?type=flat_apartment` narrows 2→1 result (Tenament excluded). BUG-2026-07-13-03.
- **Pending 4 (Fullscreen Gallery O) — WIRED to real images + live-verified.** New `getPublicListingImages(ownerType, ownerId)` in `public-search.ts` resolves the listing's `media-public` Storage rows → public URLs through the existing `media_public_read` RLS (guest-safe; only images; cover first). `DetailGallery` + `FullscreenGallery` now take `images: string[]` and render real `<img>` (draggable=false), keeping the honest placeholder when a listing has none. Property + project detail pages pass the resolved images. **Dev fixture:** 4 real stock photos seeded into `media-public` for property `a61d686c…` ("2 BHK Flat in Satellite Ahmedabad"), path-tagged `dev-fixture-*` for easy removal; property `media_count/cover_media_id` synced. Live-verified: mobile carousel, fullscreen overlay (nav + thumbnails), desktop 2fr/1fr/1fr grid, guest RLS read (4 rows), contact still masked, no console errors.
- **Pending 1 (project detail) — live-verified** (tabs Overview/Floor Plans/Amenities/Location/Gallery, construction stepper, honest provider fallbacks "no fake embed shown", masked builder contact).
- **Pending 2/3/5 — now LIVE-DRIVEN to PASS (session 2b), not just code-verified.** Logged in as testbuilder (9000000013, dev-OTP 123456) via the real OTP widget.
  - **Unit Inventory (E):** `/dashboard/builder/projects/{fcc4a368}/units` — Wing A (5×3=15, 12 units). Bulk-selected A-101 → **Mark Booked** persisted (Available→Booked live), then reverted A-101→Available. PASS.
  - **Claim (M):** as builder viewed `/broker/dev-test-realty` → "Claim this profile" → submitted (role + proof note) → "Claim request submitted". Re-submit → **dup guard** "You already have a pending claim request for this profile." PASS.
  - **Report rate-limit (N):** "Report this profile" → reason=spam → "Report submitted". Re-report → **rate-limit** "You've already reported this listing — it's with our moderation team." PASS. (Daily-cap-10 path still code-verified.)
  - Test rows created during the drive (1 claim_request + 1 user_report by the builder) were deleted afterward via service role — moderation queue left clean.
- Gates after changes: `tsc --noEmit` PASS, eslint (changed files) PASS, `npm run build` PASS. Changed: `src/lib/search/config.ts`, `src/lib/actions/public-search.ts`, `src/components/detail/DetailGallery.tsx`, `src/components/detail/FullscreenGallery.tsx`, `src/app/property/[slug]/page.tsx`, `src/app/project/[slug]/page.tsx`. No migration (RLS + bucket already existed).
- **NET RESULT: all six Phase 2 pending items = live-verified PASS.** Only remaining dev-cleanup: the gallery `dev-fixture-*` images (see [[project-gallery-dev-fixture]]) must be removed before production.
